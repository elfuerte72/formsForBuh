...
5. `app/services/sheets.py` — only place that imports `gspread` and `google.oauth2`. Column layout is fixed in `COLUMNS` (12 columns: yellow A–D from 1С, green E–K from foreman uploads, Status L). `append_row` writes a green-only row (yellow + status cells stay empty); `read_all_records` reads green-block data and skips any row whose green `№ УПД` cell (column E) is empty; `read_onec_records` reads yellow-block (1С) data and skips any row whose yellow `№ УПД` cell (column D) is empty — used to carry register rows forward across reconciliations; `rewrite_reconciliation` clears the data area below the header, batch-writes paired rows, and paints yellow/green column backgrounds via `worksheet.format`. All methods are async wrappers over `asyncio.to_thread`. SDK errors are translated into `SheetsAppendError` (writes/clears/format) or `SheetsReadError` (reads).
...
2. `app/pipelines/reconciliation.py::reconcile` — orchestrates `onec.parse(raw, filename)` → `sheets.read_all_records()` → `sheets.read_onec_records()` → `_combine_onec(...)` → `_diff(...)` → `sheets.rewrite_reconciliation(rows)` → `ReconciliationStats`. `_combine_onec` merges the freshly parsed register with the 1С rows already in the sheet so register entries **accumulate** across reconciliations: a new record wins on a `_normalize`d number clash (the export is authoritative — amounts may have been corrected), and any old register row absent from the new export is carried forward unchanged. This is why a weekly export covering only a recent window no longer wipes earlier register rows. `_diff` returns a `list[ReconRow]` plus four counts (matched / missing / extras / amount_mismatch): one `OK` row per foreman upload that matches a 1С record **and** whose amount agrees within `_AMOUNT_TOLERANCE` (0.01), one `СУММА?` row when the number matches but the amounts diverge (foreman duplicates remain as separate rows either way), one `NO` row per unmatched 1С record, one `ЛИШНЕЕ` row per foreman upload without a 1С match. Catches `OneCParseError` / `SheetsReadError` / `SheetsAppendError` / `AppError` and returns `ReconciliationResult(ok=False, error=<machine_code>)` instead of raising. Comparison key is `_normalize(upd_number)`: lowercase, strip, drop spaces, drop leading zeros — **matching is on the number only; the amount is compared but never part of the key**. The 1С column used is `Номер вх.` (incoming UPD number from supplier) — *not* the internal 1С document number. `pending_uploads(...)` reuses `read_all_records()` to count green rows with an empty Status (uploads not yet reconciled).
...
**Reconciliation is paused for stage 1.** The client asked to keep the form upload-only for now (so the foreman УПД register just accumulates and is never wiped) and to rework «Сводка» later for multiple organisations. This is gated by `reconciliation_enabled` (config, default `False`): while off, `POST /api/reconciliation` short-circuits to `error="reconciliation_disabled"` **before** any sheet access, `/api/reconciliation/pending` returns `0`, and the frontend hides the «Сводка» tab via the `RECON_ENABLED = false` constant in `index.html`. To bring reconciliation back, flip **both** flags. The reconciliation pipeline/service code stays intact for that rework.
...
- **Pydantic at every edge.** `Foreman` literal + `UploadFile` for inbound, `UPDRecord` for vision output, `DownloadedFile` for files, `UploadResult` for outbound. Stage 2 adds `OneCRecord`, `SheetUPDRow`, `ReconRow`, `ReconciliationStats`, `ReconciliationResult`. Inside the app, pass validated instances, never dicts.
