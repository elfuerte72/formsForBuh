...
2. `app/pipelines/reconciliation.py::reconcile` — orchestrates `onec.parse(raw, filename)` → `sheets.read_all_records()` → `_diff(...)` → `sheets.rewrite_reconciliation(rows)` → `ReconciliationStats`. `_diff` returns a `list[ReconRow]` plus three counts (matched / missing / extras): one `OK` row per foreman upload that matches a 1С record (foreman duplicates remain as separate `OK` rows), one `NO` row per unmatched 1С record, one `ЛИШНЕЕ` row per foreman upload without a 1С match. Catches `OneCParseError` / `SheetsReadError` / `SheetsAppendError` / `AppError` and returns `ReconciliationResult(ok=False, error=<machine_code>)` instead of raising. Comparison key is `_normalize(upd_number)`: lowercase, strip, drop spaces, drop leading zeros. The 1С column used is `Номер вх.` (incoming UPD number from supplier) — *not* the internal 1С document number.
